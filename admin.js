...
function resetAdminNoticeForm(){editingNoticeId=null;if($("adminNoticeTitle")){$("adminNoticeTitle").value="";$("adminNoticeBody").value="";$("adminNoticeTarget").value="全員";$("adminNoticeStart").value=ymd(new Date());$("adminNoticeEnd").value="";$("btnSaveAdminNotice").textContent="お知らせを追加";}}
function renderAdminNoticeList(){
  const el=$("adminNoticeList"); if(!el)return;
  const items=(data.notices||[]).slice().sort((a,b)=>String(b.createdAt||"").localeCompare(String(a.createdAt||"")));
  if(!items.length){el.innerHTML=`<div class="notice-empty">登録済みのお知らせはありません。</div>`;return}
  el.innerHTML="";
  items.forEach(n=>{
    const div=document.createElement("div");div.className=`notice-item ${n.important?"important":""}`;
    div.innerHTML=`<div class="notice-head"><div class="notice-title">${escapeHtml(n.title||"お知らせ")}</div><div class="notice-meta">${escapeHtml(n.target||"全員")} / ${escapeHtml([n.startDate||"",n.endDate||""].filter(Boolean).join(" - ")||"期限なし")}</div></div><div class="notice-body">${escapeHtml(n.body||"")}</div><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;"><button class="btn small ghost">編集</button><button class="btn small danger">削除</button></div>`;
    const btns=div.querySelectorAll("button");
    btns[0].addEventListener("click",()=>{editingNoticeId=n.id;$("adminNoticeTitle").value=n.title||"";$("adminNoticeBody").value=n.body||"";$("adminNoticeTarget").value=n.target||"全員";$("adminNoticeStart").value=n.startDate||"";$("adminNoticeEnd").value=n.endDate||"";$("btnSaveAdminNotice").textContent="お知らせを更新";window.scrollTo({top:0,behavior:"smooth"});});
    btns[1].addEventListener("click",()=>{if(!confirm("このお知らせを削除しますか？"))return;data.notices=(data.notices||[]).filter(x=>x.id!==n.id);saveData(data);renderAdminNoticeList();});
    el.appendChild(div);
  });
}

function renderAdminHome(){renderAdminNotifications();resetAdminNoticeForm();renderAdminNoticeList();const now=new Date();$("adminMonthInfo").textContent=monthLabelJa(now);$("adminTodayPassword").textContent="取得中…";fetchTodayPasswordForAdmin(false).then(p=>{$("adminTodayPassword").textContent=p||"―";}).catch(()=>{$("adminTodayPassword").textContent="―";});const users=Object.entries(data.users||{}).map(([id,u])=>{u=u||{}; if(!u.id) u.id=id; return u;}).filter(u=>u.userType==="学生").sort((a,b)=>(a.createdAt||0)-(b.createdAt||0));const tb=$("adminTbody");tb.innerHTML="";
...
$("btnSaveAdminNotice").addEventListener("click",()=>{
  const title=$("adminNoticeTitle").value.trim();
  const body=$("adminNoticeBody").value.trim();
  if(!title||!body){showModal({title:"入力不足",sub:"タイトルと本文を入力してください",big:"⚠️"});return}
  data.notices=data.notices||[];
  const notice={
    id: editingNoticeId || ("notice_"+Date.now()),
    title:title,
    body:body,
    target: $("adminNoticeTarget").value,
    startDate: $("adminNoticeStart").value,
    endDate: $("adminNoticeEnd").value,
    important: false,
    createdAt: editingNoticeId ? ((data.notices.find(x=>x.id===editingNoticeId)||{}).createdAt||new Date().toISOString()) : new Date().toISOString(),
    updatedAt: new Date().toISOString()
  };
  const idx=(data.notices||[]).findIndex(x=>x.id===notice.id);
  if(idx>=0)data.notices[idx]=Object.assign({},data.notices[idx],notice);
  else data.notices.unshift(notice);
  saveData(data);resetAdminNoticeForm();renderAdminNoticeList();showModal({title:"保存しました",big:"📢"});
});
$("btnClearAdminNotice").addEventListener("click",resetAdminNoticeForm);
